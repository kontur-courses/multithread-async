...
public class ClusterServer
{
    ...
    private Action<HttpListenerContext> CreateSyncCallback(int methodDuration, int status = 200)
    {
        return context =>
        {
            ...
            log.InfoFormat(
                $"Thread #{Thread.CurrentThread.ManagedThreadId} received request '{query}' #{currentRequestId} at {DateTime.Now.TimeOfDay}");
            ...
            log.InfoFormat(
                $"Thread #{query} sent response {status} for '{Thread.CurrentThread.ManagedThreadId}' for #{currentRequestId} at {DateTime.Now.TimeOfDay}");
        };
    }
    ...
    private Func<HttpListenerContext, Task> CreateAsyncCallback(int methodDuration, int status = 200)
    {
        return async context =>
        {
            ...
            var query = context.Request.QueryString["query"];
            log.InfoFormat(
                $"Thread #{Thread.CurrentThread.ManagedThreadId} received request '{query}' #{currentRequestNum} at {DateTime.Now.TimeOfDay}");
            ...
            log.InfoFormat(
                $"Thread #{Thread.CurrentThread.ManagedThreadId} sent response {status} for '{query}' #{currentRequestNum} at {DateTime.Now.TimeOfDay}");
        };
    }
}