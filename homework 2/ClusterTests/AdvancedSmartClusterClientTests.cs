...
public class AdvancedSmartClusterClientTests : SmartClusterClientTest
{
    protected override ClusterClientBase CreateClient(string[] replicaAddresses)
        => new AdvancedSmartClusterClient(replicaAddresses);

    [Test]
    public override void ShouldNotForgetPreviousAttemptWhenStartNew()
    {
        CreateServer(4500);
        CreateServer(3000);
        CreateServer(10000);

        ProcessRequest(CreateClient(), "one", 6000).Should().BeCloseTo(TimeSpan.FromMilliseconds(4500), Epsilon);
    }
    
    [Test]
    public override void ShouldNotSpendTimeOnBad()
    {
        ...
        ProcessRequest(CreateClient(), "one", 6000).Should().BeCloseTo(TimeSpan.FromMilliseconds(4000), Epsilon);
    }
    ...
    public void ShouldSortReplicasByReplyTime()
    {
        ...
        ProcessRequest(client, "first", timeout).Should().BeCloseTo(TimeSpan.FromMilliseconds(4000), Epsilon);
